...
public class GetPopularRequestTaskListDtoConverter {
	...
	public static GetPopularRequestTaskListDto.Response toResponse(Page<Object[]> requestTasks) {

		List<GetPopularRequestTaskListDto.requestTaskDetail> requestTaskDetailList = requestTasks.stream()
			.map(requestTask -> {
					...
					return toRequestTaskDetail(task, saveCount, days);
				}
			).toList();

		return GetPopularRequestTaskListDto.Response.builder()
			.requestTaskDetailList(requestTaskDetailList)
			.totalPages(requestTasks.getTotalPages())
			.totalElements((int)requestTasks.getTotalElements())
			.currentPage(requestTasks.getNumber())
			.currentSize(requestTasks.getNumberOfElements())
			.hasNext(requestTasks.hasNext())
			.build();
	}

	public static GetPopularRequestTaskListDto.requestTaskDetail toRequestTaskDetail(RequestTask requestTask, Long saveCount,
		Long deadline) {
