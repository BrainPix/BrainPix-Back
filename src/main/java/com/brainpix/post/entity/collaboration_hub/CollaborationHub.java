...
public class CollaborationHub extends Post {
	...
	public Map<String, Long> calculateTotalAndOccupiedParticipants() {
		if (collaborations.isEmpty()) {
			throw new IllegalStateException("No recruitments found for the collaboration hub.");
		}

		Map<String, Long> result = new HashMap<>();
		result.put("total", getTotalQuantity());
		result.put("occupied", getOccupiedQuantity());
		return result;
	}
	...
	public long calculateDaysLeft() {
		return Math.max(this.deadline.toLocalDate().toEpochDay() - LocalDate.now().toEpochDay(), 0);
	}
